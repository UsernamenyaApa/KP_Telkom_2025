...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="min-h-screen bg-gradient-to-br from-blue-50 to-blue-100 dark:from-blue-900 dark:to-blue-800" x-data="{
            sidebarOpen: false,
            get darkMode() {
        ...
        ">
        <flux:sidebar
            class="fixed inset-y-0 left-0 z-50 w-64 border-e border-zinc-200 bg-white dark:border-zinc-700 dark:bg-zinc-800"
            x-show="sidebarOpen"
        ...
        >
            <flux:sidebar.toggle icon="x-mark" @click="sidebarOpen = false" class="text-gray-900 dark:text-white hover:text-blue-500 dark:hover:text-blue-200" />

            <a href="{{ route('dashboard') }}" class="me-5 flex items-center space-x-2 rtl:space-x-reverse mb-6 p-3 rounded-lg hover:bg-gray-100 dark:hover:bg-blue-700" wire:navigate>
            ...
            <flux:navlist variant="outline" class="[&_*]:text-gray-900 [&_*]:dark:text-white [&_*]:border-blue-400">
                <flux:navlist.group :heading="__('Platform')" class="grid [&_h3]:text-blue-200 [&_h3]:font-semibold [&_h3]:text-sm [&_h3]:uppercase [&_h3]:tracking-wider">
                    <flux:navlist.item 
                        icon="home" 
                ...
                @if (auth()->user()->hasRole('super-admin'))
                    <flux:navlist.group :heading="__('Admin')" class="grid [&_h3]:text-blue-200 [&_h3]:font-semibold [&_h3]:text-sm [&_h3]:uppercase [&_h3]:tracking-wider [&_h3]:mt-6">
                        <flux:navlist.item 
                            icon="server" 
        ...
        <flux:header class="fixed top-0 w-full z-40 bg-white dark:bg-zinc-800 shadow-lg border-b border-gray-200 dark:border-zinc-700">
            <flux:sidebar.toggle icon="bars-2" inset="left" @click="sidebarOpen = true" class="text-gray-900 dark:text-white hover:text-blue-500 dark:hover:text-blue-200" />
            ...
            <div class="flex items-center space-x-3 me-3">
                ...
                <flux:dropdown position="top" align="end">
                    <flux:profile
                        ...
                        class="text-gray-900 dark:text-white border-blue-400 hover:border-blue-300"
                    />

                    <flux:menu class="bg-white dark:bg-blue-800 border-blue-200 dark:border-blue-600">
                        ...
                        <flux:menu.radio.group>
                            <flux:menu.item :href="route('settings.profile')" icon="cog" wire:navigate class="text-gray-700 dark:text-white hover:bg-blue-50 dark:hover:bg-blue-700">{{ __('Settings') }}</flux:menu.item>
                        </flux:menu.radio.group>
                        ...
                        <form method="POST" action="{{ route('logout') }}" class="w-full">
                            @csrf
                            <flux:menu.item as="button" type="submit" icon="arrow-right-start-on-rectangle" class="w-full text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900 dark:hover:bg-opacity-20">
                                {{ __('Log Out') }}
                            </flux:menu.item>
                        </form>
                    </flux:menu>
        ...
        <div class="bg-gradient-to-br from-blue-50 to-blue-100 dark:from-blue-900 dark:to-blue-800 pt-16 min-h-screen" :class="{'lg:ms-64': sidebarOpen, 'lg:ms-0': !sidebarOpen}">
            {{ $slot }}
        </div>
